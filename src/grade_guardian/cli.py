...
def test_scrape(ctx, student_name: Optional[str], output_dir: Optional[str], run_scraper: bool):
    ...
    try:
        ...
        click.echo(f"✅ Scrape completed successfully!")
...
def run_docker_scraper(output_path: Path, student_name: Optional[str]) -> Optional[Path]:
...
def create_sample_data(output_path: Path, student_name: Optional[str]) -> Path:
...
def init_db(ctx):
    ...
    if db.test_connection():
        ...
        if health_info.get('status') == 'healthy':
            stats = health_info['stats']
            click.echo(f"Current data: {stats['student_count']} students, {stats['assignment_count']} assignments")
    else:
        click.echo("❌ Database connection failed")
...
@click.pass_context  
def create_config(ctx, config_path: Optional[str]):
    """Create a default configuration file."""
    if config_path:
        path = Path(config_path)
    else:
        path = Path("data/config/config.yaml")
