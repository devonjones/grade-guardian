...
class GradeDataProcessor:
    ...
    def process_scraped_file(self, json_file_path: Path) -> dict[str, Any]:
        ...
        try:
            ...
            with self.db.get_connection() as conn:
                ...
                student_id = self.find_or_create_student_with_cursor(
                    cursor, data.get("student", "Unknown")
                )
                if not student_id:
                ...
                for course_data in data.get("courses", []):
                    try:
                        course_result = self.process_course_with_cursor(
                            cursor, student_id, course_data
                        )
                        processing_result["courses_processed"] += 1
                        processing_result["assignments_processed"] += course_result[
                            "assignments_processed"
                        ]
                        processing_result["new_assignments"] += course_result["new_assignments"]
                        processing_result["updated_assignments"] += course_result[
                            "updated_assignments"
                        ]
    ...
    def validate_assignment_data(
        self, assignment: dict[str, Any], course_index: int, assignment_index: int
    ) -> list[str]:
    ...
    def find_or_create_student(self, student_name: str) -> int | None:
    ...
    def find_or_create_student_with_cursor(self, cursor, student_name: str) -> int | None:
    ...
    def process_course_with_cursor(
        self, cursor, student_id: int, course_data: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        for assignment_data in course_data.get("assignments", []):
            try:
                assignment_result = self.process_assignment_with_cursor(
                    cursor, course_id, assignment_data
                )
                ...
                logger.error(
                    f"Error processing assignment {assignment_data.get('name', 'Unknown')}: {e}"
                )
                raise

        return result

    def find_or_create_course(self, student_id: int, course_data: dict[str, Any]) -> int | None:
    ...
    def find_or_create_course_with_cursor(
        self, cursor, student_id: int, course_data: dict[str, Any]
    ) -> int | None:
    ...
    def process_assignment_with_cursor(
        self, cursor, course_id: int, assignment_data: dict[str, Any]
    ) -> dict[str, Any]:
        """Process a single assignment and its grade history using provided cursor."""
        try:
            # Find or create assignment
            assignment_result = self.find_or_create_assignment_with_cursor(
                cursor, course_id, assignment_data
            )
            ...
            grade_changed = self.process_grade_history_with_cursor(
                cursor, assignment_id, assignment_data
            )

            return {"is_new": is_new, "grade_changed": grade_changed}
    ...
    def find_or_create_assignment_with_cursor(
        self, cursor, course_id: int, assignment_data: dict[str, Any]
    ) -> tuple[int, bool] | None:
    ...
    def process_grade_history(
        self, cursor, assignment_id: int, assignment_data: dict[str, Any]
    ) -> bool:
    ...
    def process_grade_history_with_cursor(
        self, cursor, assignment_id: int, assignment_data: dict[str, Any]
    ) -> bool:
        ...
        try:
            ...
            cursor.execute(
                ...
                AND status = %(status)s
            """,
                {"assignment_id": assignment_id, "points_earned": points_earned, "status": status},
            )
